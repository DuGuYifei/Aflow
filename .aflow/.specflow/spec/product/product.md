...
未来的 Aflow 是 Specflow 之上的 TUI agent cockpit：它使用 `coding-agent` 作为 agent SDK，重写系统提示词、命令和 TUI 体验，通过 Specflow server 创建、改写、验证、运行和恢复 workflow，并在运行结束后提供原生 agent CLI 的 terminal handoff 能力。详细方向见 [aflow-agent-cockpit.md](./aflow-agent-cockpit.md)。
