...
未来的 Aflow 是 Specflow 之上的 TUI agent cockpit：它使用 `@earendil-works/pi-coding-agent` 作为 agent SDK，重写系统提示词、命令和 TUI 体验，通过 Specflow server 创建、改写、验证、运行和恢复 workflow，并在运行结束后提供原生 agent CLI 的 terminal handoff 能力。详细方向见 [aflow-agent-cockpit.md](./aflow-agent-cockpit.md)。
...
**Workflow / AgentFlow** 是把输入转化为 Agent 工作结果的结构化流程。它不是简单线性步骤，而是由节点和边组成的图；共享可编辑源码保存在 `.aflow/.specflow/agentflows/*.yaml`，本地草稿和 fork/adapt 变体保存在 gitignored 的 `.aflow/.specflow/agentflows-local/*.yaml`，浏览器画布布局单独保存在 `.aflow/.specflow/canvas/*.json`。
