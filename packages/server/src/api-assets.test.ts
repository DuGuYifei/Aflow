...
describe("canvas asset API", () => {
  test("stores image inputs as stable workflow assets", async () => {
    ...
    expect(body.images[0]!.path).toContain(".aflow/.specflow/agentflow/assets/wf/images/");
  ...
  test("preserves selected folder paths under workflow resources", async () => {
    ...
    expect(response?.status).toBe(200);
    expect(body.paths).toEqual([".aflow/.specflow/agentflow/assets/wf/resources/components/"]);
    expect(await readFile(join(root, ".aflow/.specflow/agentflow/assets/wf/resources/components/button.ts"), "utf8")).toBe("button");
    expect(await readFile(join(root, ".aflow/.specflow/agentflow/assets/wf/resources/components/cards/card.ts"), "utf8")).toBe("card");
  });
});
