...
Server 是本地 server / IPC 适配层，不是 hosted backend，也不是传统前后端分离架构里的远程后端服务。未来它可以连接 CLI、桌面壳、UI 面板和 runtime。
...
`packages/server` 存放本地 server / IPC 适配层占位。
...
Specflow 可以有多个本地产品表面：CLI、未来桌面壳、UI 面板、本地 server 或 IPC。它们都应该围绕同一个 runtime 工作，而不是形成传统前后端分离架构。
