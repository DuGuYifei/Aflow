...
初始 `apps/cli + apps/server + apps/web` 结构容易让架构重心偏向独立后端和独立前端。实际产品应该以本地 runtime 为中心，CLI、UI、Server、未来桌面壳都只是围绕 runtime 的产品表面。
...
本地产品表面放在 `packages/server` 和 `packages/ui`。它们是可构建能力包，不是独立后端或独立 Web 产品。
