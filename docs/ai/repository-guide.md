...
Session Director 的 mock session decision 应从 definition 派生，读取 managed node 顺序、session group、session mode 和 `newSessionOnLoop`。不要把某个 Phase 1 节点 id 写死为“必须新开 session”。
...
`reviewerMode` 和 `maxRepairAttempts` 是 mock run options，只用于本地 placeholder review/repair 调试。不要把这些选项写进 workflow definition schema。
...
UI 不应自行推导节点执行模式、agent CLI、session controller、control scope 摘要或反向 managed-by 关系。需要展示 definition 级执行信息时，应消费 runtime 派生的 execution preview。
