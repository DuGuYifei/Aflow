...
Workflow-as-code 文件保存在 `.aflow/.specflow/agentflow/agentflows/*.yaml`。

浏览器画布布局生成到 `.aflow/.specflow/agentflow/canvas/*.json`。

Agent server 配置在 `.aflow/.specflow/agent-servers.json` 中。本地密钥和机器相关覆盖项写入 `.aflow/.specflow/agent-servers.local.json`。

Run 记录、run 日志和 workflow 资源也保存在 `.aflow/.specflow/` 下。

可复用的 ACP agent cache 默认保存在 `~/.aflow/.specflow/cache/agents/`，也可以通过 `SPECFLOW_AGENT_CACHE_DIR` 移到其他位置。
...
```sh
curl -fsSL https://raw.githubusercontent.com/DuGuYifei/Aflow/master/install/install.sh | bash
```

默认情况下，安装脚本优先解析最新的稳定版 `vX.Y.Z`。如果当前还没有稳定版，会回退到最新的 semver prerelease，例如 `vX.Y.Z-beta.1`。如果要安装指定 release，在 `bash -s --` 后面传入 tag：

```sh
curl -fsSL https://raw.githubusercontent.com/DuGuYifei/Aflow/master/install/install.sh | bash -s -- v0.0.1-beta.2
```

Windows PowerShell：

```powershell
& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/DuGuYifei/Aflow/master/install/install.ps1")))
```

Windows 安装指定 release：

```powershell
& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/DuGuYifei/Aflow/master/install/install.ps1"))) "v0.0.1-beta.2"
```

安装脚本从 `master` 加载，并默认解析最新稳定版。真正的 release binaries 会挂在 `vX.Y.Z` 以及 `vX.Y.Z-alpha.1`、`vX.Y.Z-beta.1`、`vX.Y.Z-rc.1` 这类 prerelease tag 下。
...
该命令会构建内嵌 UI 资源，然后启动 Aflow。无参数启动时，Aflow 会显示模式选择：
...
选择 Specflow 会启动本地 Specflow server 并打开 workspace UI；选择 Designer 会启动同一个 server 并打开 `/design`。生产模式下，server 会从编译后的二进制中提供内嵌的静态 UI。

## Scripts

```sh
bun run dev        # 构建 UI 资源，然后启动 Aflow 模式选择
