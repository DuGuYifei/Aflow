...
Execution preview 由 `packages/runtime` 从 workflow definition 派生，包含节点执行模式、agent CLI、session policy、controller、control scope 摘要，以及被哪些 controller 管理的反向关系。Server 不应重新推导，UI 不应复制 runtime 规则。
