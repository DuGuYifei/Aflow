...
- **存储**:`.specflow/cache/agents/capabilities.json`,由 [AgentServerStore](../../packages/agent-proxy/src/store/agent-server-store.ts) 读写。结构 `AgentServerCapabilitiesCache`:`agentCapabilities` / `modes` / `configOptions` / `availableCommands` + `installedVersion` + `probedAt`。
