# Workflow 状态目录

这个目录只记录当前仓库的 workflow 生效状态和边界。

完整阶段路线见 `docs/product/roadmap.md`。

当前阶段状态见 `.specflow/workflows/phase-1-local-loop.md`。

结构化 workflow definition 使用 `*.workflow.json`。这类文件记录当前可验证的 session group、节点、边、节点 agent CLI、session policy 和 control scope，是未来 UI 编辑 workflow 的输入边界。

当前 schema 位于 `.specflow/workflows/workflow-definition.schema.json`。仓库内的 workflow definition 应通过 `$schema` 指向它，便于编辑器和未来脚手架复用。

后续阶段说明见 `docs/product/phases/`。

不要在这里重复产品路线、阶段解释或长篇设计背景。长解释放 `docs`，这里保留当前生效事实。
