...
> `available_commands_update` 返回的 slash command 快照，供 Specflow 后续设计
> command discovery、命令输入 UI 和更多 CLI 兼容性验证时参考。
>
> 探测日期：2026-05-24。
>
...
- 当前 agent-proxy 已能通过 `onSessionUpdate` 接收该通知；UI 还没有将命令列表暴露为 slash command 菜单或输入提示。
